...
from redmine.utils import load_generic_object
from redmine.managers import IssueManager
...
class CustomField(models.Model):
    ...
    field_format = models.CharField(max_length=30, choices=FORMAT_CHOICES)
...
class Issue(models.Model):
    ...
    def custom_field(self, name):
        try:
            return CustomValue.objects.get(customized_type="Issue",
                                           customized_id=self.pk,
                                           custom_field__name=name).value
...
class Project(models.Model):
    ...
    def custom_field(self, name):
        try:
            return CustomValue.objects.get(customized_type="Project",
                                           customized_id=self.pk,
                                           custom_field__name=name).value
        except CustomValue.DoesNotExist:
            return None
